...
'''A tool create rules for Iptables that block
or allow networks by country code(ex: RU CN etc.).
For the correct execution of script need to download geip
...
def main():
    ...
    p.add_option("--allow_only", "-a",
                 help='Generate iptables rules that allow '
                      'only selected coutry',
                 action="store_true")
    ...
    if options.cc:
        ...
        with open(options.countrydb) as f:
            for line in f:
                if line == "" or line.startswith("Country ") \
                        or ";" not in line:
                    continue
